...
func ResolveChannel(apiKey, channelURL string) (*Channel, error) {
	...
	case "@":
		...
		return fetchChannelByHandle(apiKey, segments[0])
	default:
...
func fetchChannelByID(apiKey, channelID string) (*Channel, error) {
...
func fetchChannelByHandle(apiKey, handle string) (*Channel, error) {
...
func FetchVideos(apiKey, channelID string, maxResults int) ([]Video, error) {
...
func FetchDescriptions(apiKey string, videoIDs []string) (map[string]string, error) {
	if len(videoIDs) == 0 {
...
func FetchChannel(apiKey, channelURL string, maxResults int) (*Channel, []Video, error) {
	ch, err := ResolveChannel(apiKey, channelURL)
	if err != nil {
		return nil, nil, fmt.Errorf("resolving channel: %w", err)
	}

	videos, err := FetchVideos(apiKey, ch.ID, maxResults)
	if err != nil {
		return nil, nil, fmt.Errorf("fetching videos: %w", err)
	}
	...
	descs, err := FetchDescriptions(apiKey, videoIDs)
	if err != nil {
		return nil, nil, fmt.Errorf("fetching descriptions: %w", err)
	}
