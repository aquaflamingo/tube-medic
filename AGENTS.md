...
| Module | Path | Purpose |
|--------|------|---------|
| **core** (`github.com/aqfl/tmcore`) | `./tube-medic-core/` | Public API: `Config`, `Report`, `RunScan`. Scans YouTube channel descriptions for broken/suspicious links. |
| **web** (`github.com/aqfl/tubemedic-web`) | `./web/` | Web UI (HTMX + Tailwind). Importes `tmcore` and calls `RunScan`. |
| **leadbot** (`github.com/aqfl/tmleadbot`) | `./leadbot/` | Lead-finding pipeline for YouTube creator outreach. |
...
The `web/internal/handlers/handlers.go` was rewired to import `github.com/aqfl/tmcore`
instead of the old `github.com/aqfl/tmcore`.
...
- `web/go.mod` now depends on `github.com/aqfl/tmcore` (resolved locally via workspace)
